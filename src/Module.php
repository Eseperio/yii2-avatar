...
class Module extends \yii\base\Module
{
     ...
     * - `closure` a custom function that returns a unique identifier.Signature: `function($avatarModule){}`
    ...
    public function getAvatarFileName($suffix = null)
    {
        $name = [];
        $userComponent = Yii::$app->get($this->userComponent);

        if (empty($this->avatarFileName)) {
            $name = [$userComponent->id];

        } elseif (is_string($this->avatarFileName)) {
            $name = ArrayHelper::getValue($userComponent, $this->avatarFileName);
        } elseif (is_array($this->avatarFileName) || $this->avatarFileName instanceof \Closure) {
            call_user_func($this->avatarFileName, $this);
        }
